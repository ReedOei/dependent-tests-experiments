...
public class SFSReduction {
    public static void main(String[] args) {
        if (args.length > 1) {
            ...
            System.out
                    .println("\tjava gov.nasa.ltl.graph.SFSReduction [<filename>]");
    ...
    public static Graph reduce(Graph g) {
        // debugged by Dimitra 3/4/02 - added |PO| information so that
        // synoptic.main while
        ...
        while ((currNumColors != prevNumColors) || (currNumPO != prevNumPO)) {
            ...
            newColorList = new LinkedList<Pair<ColorPair>>(); // keeps
                                                              // association of
                                                              // node with new
                                                              // color
            ...
            for (Iterator<Node> i = nodes.iterator(); i.hasNext();) {
                ...
                ColorPair currPair = new ColorPair(
                        currNode.getIntAttribute("_prevColor"), getPrevN(
                                currNode, prevPO));

                /*
                 * System.out.println("Transition set from node: " +
                 * currNode.getId() + " is: " + currPair.getIMaxSet());
                 */
                newColorList
                        .add(new Pair<ColorPair>(currNode.getId(), currPair));
            ...
            // "\n");
            // Dimitra comments
            // Convert the set into a linked list so that rank of object is
            // known
            ...
            // Renaming color set
            for (Iterator<Pair<ColorPair>> i = newColorList.iterator(); i
                    .hasNext();) {
            ...
            for (Iterator<Pair<ColorPair>> i = newColorList.iterator(); i
                ...
                for (Iterator<Pair<ColorPair>> j = newColorList.iterator(); j
                        .hasNext();) {
                    ColorPair currPairTwo = j.next().getElement();
                    boolean po = prevPO[currPairTwo.getColor() - 1][currPairOne
                            .getColor() - 1];
                    boolean dominate = iDominateSet(currPairOne.getIMaxSet(),
                            currPairTwo.getIMaxSet(), prevPO);

                    if (po && dominate) {
                        currPO[ordered.indexOf(currPairTwo)][ordered
                                .indexOf(currPairOne)] = true;
                        currNumPO++;
                    } else {
                        currPO[ordered.indexOf(currPairTwo)][ordered
                                .indexOf(currPairOne)] = false;
                    }
                }
            }
        }
        ...
        if (newColorList == null) {
            ...
            for (Iterator<Pair<ColorPair>> i = newColorList.iterator(); i
                ...
                if (newColorSet.contains(colPair)) {
                    // for all transitions based on colors, newColorSet makes
                    // sure that
                    // no duplicates exist, neither transitions that dominate
                    // each other
                    // that is why we only add transitions that belong to it;
                    // I guess we could also just use all transitions in
                    // newColorSet to
                    ...
                    for (Iterator<ITypeNeighbor> j = pairSet.iterator(); j
                            .hasNext();) {
                        ITypeNeighbor neigh = j.next();
                        int neighPos = neigh.getColor() - 1;
                        @SuppressWarnings("unused")
                        Edge e = new Edge(currNode, newNodes[neighPos],
                                neigh.getTransition());
                    }
    ...
    private static TreeSet<ITypeNeighbor> getPrevN(Node currNode,
            boolean[][] prevPO) {
        ...
        for (Iterator<Edge> i = edges.iterator(); i.hasNext();) {
            ...
            iNeigh = new ITypeNeighbor(currEdge.getNext().getIntAttribute(
                    "_prevColor"), currEdge.getGuard());
    ...
    private static boolean iDominateSet(TreeSet<ITypeNeighbor> setOne,
            TreeSet<ITypeNeighbor> setTwo, boolean[][] prevPO) {
        ...
        for (Iterator<ITypeNeighbor> i = working.iterator(); i.hasNext();) {
            ...
            for (Iterator<ITypeNeighbor> j = setOne.iterator(); j.hasNext();) {
                ...
                ITypeNeighbor dominating = iDominates(neighOne, neighTwo,
                        prevPO);
    ...
    /**
     * Returns the neighbor that dominates. If none dominates the other, then
     * returns null
     */
    private static ITypeNeighbor iDominates(ITypeNeighbor iNeigh,
            ITypeNeighbor nNeigh, boolean[][] prevPO) {
        ...
        while (!work.isEmpty()) {
            ...
            if (currNode != null) {
                ...
                for (Iterator<Edge> i = outgoingEdges.iterator(); i.hasNext();) {
                    ...
                    if (!(work.contains(nextNode) || reachable
                            .contains(nextNode))) {
                        work.add(nextNode);
                    }
                }
